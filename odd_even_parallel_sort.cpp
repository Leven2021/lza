...
int main (int argc, char **argv) {
    ...
    */
    ...
    // construct two new arrays to store old elements plus some zeros, this is for indivisible case only!!!
    int num_zeros = (num_elements % world_size == 0) ? 1 - num_elements : world_size - (num_elements % world_size);
    int new_elements[num_elements + num_zeros];
    int new_sorted_elements[num_elements + num_zeros];
    ...
    if (num_elements % world_size == 0) {
        ...
        for (int j = 0; j < num_elements + num_zeros; j++) {
            if (j < num_zeros) {
                new_elements[j] = 0;
            } else {
                new_elements[j] = elements[j - num_zeros];
            }
        }
    ...
    if (num_my_element % 2 == 0) { // every process has even elements
        while (true) {
            ...
            // send the left boundary element to the anterior process, compare, and then send something back
            if (rank != 0) {
    ...
    if (num_elements % world_size == 0) {
        ...
        for (int j = 0; j < num_elements; j++) {
            sorted_elements[j] = new_sorted_elements[j + num_zeros]; // here we are sure that zero must be the smallest
        }
    }
